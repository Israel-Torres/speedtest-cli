...
######################## Begin ########################
# Final Output speedtestRR (Re-wRitten by Israel Torres)
# 20150721 - code optimized for import use from another python program and csv style output
def speedtestRR():
    ...
    try:
        ...
        print_('Cannot retrieve speedtest configuration - aborting the matrix')
        sys.exit(1)
    servers = closestServers(config['client'])
    fromlocationaval = '%(isp)s (%(ip)s)' % config['client']
    best = getBestServer(servers)
    tolocationval = '%(sponsor)s (%(name)s) [%(d)0.2f km]' % best
    latencyval = '%(latency)s ms' % best
    ...
    dlspeed = downloadSpeed(urls, True) #no dots
    dlspeedval = '%0.2f M%s/s' % ((dlspeed / 1000 / 1000) * 8, 'bit')
    sizesizes = [int(.25 * 1000 * 1000), int(.5 * 1000 * 1000)]
    sizes = []
    for size in sizesizes:
        for i in range(0, 25):
            sizes.append(size)
    ulspeed = uploadSpeed(best['url'], sizes, True) #no dots
    ulspeedval = '%0.2f M%s/s' % ((ulspeed / 1000 / 1000) * 8, 'bit')
    timestamp = time.strftime("%Y%m%d:%H:%M:%S")
    #"Date","fromLocation","toLocation","Latency","Download","Upload"
    print_ ('"',timestamp,'"', ',','"', fromlocationaval,'"', ',','"', tolocationval,'"', ',','"', latencyval,'"', ',','"', dlspeedval,'"',',','"', ulspeedval,'"', sep='')
######################## End ########################
